...
- **帖子存在性校验（依赖 Feed 服务）**：`feed 必须存在` 通过 `FeedRpc.GetFeed` 校验；Feed 服务返回「帖子不存在」时映射为 `CommentFeedNotFound=13002`。该下游依赖原 `00-overview.md` / `01-data-model.md` 未显式列出，实现时已补入 `00-overview.md` 依赖表。
